...
/**
 * This converter helps to cut images into pieces for the puzzles and to create thumbnail images.
 */
...
function writeSvelteFile(filename: string, numCols: number, numRows: number) {
	...
	const pageDir = `../src/routes/puzzle/${filename}`;
	fs.mkdirSync(pageDir, { recursive: true });
...
async function convertAll() {
	...
	const outputDirRoot = '../static/assets/puzzle';
	fs.mkdirSync(outputDirRoot, { recursive: true });
	fs.writeFileSync(
		`../src/routes/puzzle/puzzle-pages.ts`,
	...
	for (let i = 0; i < files.length; i++) {
		...
		await convertFile(`${inputDir}/${file}`, outputDir, size, filenameWithoutExtension);
	}
}
